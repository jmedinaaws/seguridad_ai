...
chroma_path = os.environ.get("CHROMA_PATH"+"2")
excel_file_path = os.environ.get("EXCEL_FILE_PATH")
ef = embedding_functions.SentenceTransformerEmbeddingFunction(model_name="multi-qa-MiniLM-L6-dot-v1")
...
# Iterate over each row of the excel file
    for index, row in df.iterrows():
        ...
        if           len(str(row["Pregunta"]) )> 30 and len(str(row["Detalle"]) ) >30:
...
def add_to_chroma(documents, ids):
    ...
    #)
    client = chromadb.PersistentClient(path=chroma_path, embedding_function=ef)
    db = client.get_or_create_collection(name="seguridad_db")
